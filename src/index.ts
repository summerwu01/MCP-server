...
class DuckDuckGoServer {
  ...
  private async searchDuckDuckGo(query: string, count: number = 5): Promise<SearchResult[]> {
    try {
      // 创建结果数组
      ...
      throw new McpError(
        ...
        `Failed to search DuckDuckGo: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
